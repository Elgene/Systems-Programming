 ...
 * needed in this program. Open server2.h for more
 * details.
 */
#include "server.h"
...
/* Step 1 : create a socket with the socket() system call */
  ...
  //create the struct for server address
...
/* Step 2 : Bind the socket to an address using the bind() system call */
  ...
  //and wait until it is available)
    ...
    //set the informing buffer
     strncpy(incoming, "HELLO\n", 7);
     send(client_fd,incoming,strlen(incoming), 0);
    //create fork for multiple connection for client
     pid = fork();
     if (pid < 0) {
    ...
    else continue; //continue when fork is failed to create 
  }
}
return 0;
